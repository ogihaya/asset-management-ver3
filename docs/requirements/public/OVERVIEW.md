...
資産項目は全カテゴリ共通でライフサイクル管理し、追加した月から毎月の入力項目として自動表示する。
...
| 2.2 | 資産ライフサイクル管理 | 資産の追加後は月次入力項目へ継続表示し、削除時は最新の確定月まで保持したうえで次の未確定月から非表示にする。削除済み資産は復活でき、復活時は最新の確定月の次の未確定月から再表示する。 | Must |
...
- 資産ライフサイクル（追加/削除/復活）は全カテゴリに同一ルールで適用する。
