...
- `将来収支と投資判断カード` では `投資可能額` を主役の指標として扱い、将来収支予測はその判断材料として同一カード内に補助表示する。
...
| 9 | 将来収支と投資判断カード | 情報カード | - | 投資可能額、累積収入、累積支出、余力資産、生活防衛資金、採用イベント、投資先ごとの配分結果を主表示し、その下に予測月次収入、予測月次支出、平均対象月数を判断材料として補助表示する。採用イベントは `年月 + 内容` のみとし、複数イベント比較一覧は表示しない。平均対象月数は補足情報として扱う。 |
