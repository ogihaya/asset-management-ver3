...
- `将来収支と投資判断カード` では `投資可能額` を主役の指標として扱う。
...
- 最下段に `資産グラフカード` を配置する。
- モバイルでは PC と同じ順序で各カードを縦積みとする。

#### 表示項目
| # | 項目名 | 型 | 必須 | 説明 |
|---|---|---|---|---|
| 1 | 分析対象月表示 | ステータス表示 | - | 最新の確定済み月を表示する。 |
| 2 | 総資産 | KPIカード | - | 最新確定月の総資産を表示する。総資産は通常資産合計と投資評価額合計の合計とし、クリック操作は持たない。 |
| 3 | 月次収入 | KPIカード | - | 最新確定月の収入合計を表示する。クリック操作は持たない。 |
| 4 | 推定/補正後支出 | KPIカード | - | 最新確定月の確定支出を表示する。クリック操作は持たない。 |
| 5 | 投資可能額 | KPIカード | - | 全ライフイベント比較で最小となる投資可能額を表示する。クリック操作は持たない。 |
| 6 | 資産サマリーカード | 情報カード | - | 総資産、通常資産合計、投資評価額合計、収入、支出、通常資産上位3件を表示する。総資産は通常資産合計と投資評価額合計の合計として示し、通常資産上位3件は総資産内比率付きで表示する。負債も絶対値で含め、`負債` ラベルと色分けで区別する。 |
| 7 | 通常資産全件モーダル導線 | ボタン | - | `残りn件の通常資産` とあわせて、最新確定月で有効な通常資産全件を一覧だけのモーダルで開く。投資先はこのモーダルに含めない。 |
| 8 | 投資構成カード | 情報カード | - | 最新確定月の投資評価額構成比円グラフ、実績比率、理想比率、差分、評価額を表示する。投資先一覧は設定画面で定義した配分順を維持し、比率は小数第2位まで表示する。差分は数値と色付きバッジで示す。 |
| 9 | 将来収支と投資判断カード | 情報カード | - | 予測月次収入、予測月次支出、平均対象月数、投資可能額、累積収入、累積支出、余力資産、生活防衛資金、採用イベントに加え、投資先ごとの配分結果を表示する。配分結果は最新確定月の投資評価額を基準に、投資後の理想額に対する不足額へ寄せるリバランス方向で算出する。採用イベントは `年月 + 内容` のみとし、複数イベント比較一覧は表示しない。平均対象月数は補足情報として扱う。 |
| 10 | 資産グラフカード | グラフカード | - | 単一対象の資産推移グラフを表示する。初期表示は `総資産・6か月` とし、収入と支出も正の値として表示する。 |
| 11 | 表示対象選択 | セレクト | - | カテゴリ見出し付きセレクトで `総資産 → 収入 → 支出 → 個別項目` の順で選択する。個別項目には通常資産、投資先、履歴対象を含み、履歴対象は専用グループに分ける。 |
| 12 | 表示期間切替 | ボタン群 | - | `3か月 / 6か月 / 1年 / 全期間` を切り替える。履歴が不足していてもボタンは表示し続け、存在分だけ描画する。 |
| 13 | グラフツールチップ | 補助表示 | - | ホバー時に `年月 + 値` のみを表示する。 |
...
- KPIカードは表示専用とし、クリックやドリルダウン操作は持たない。
- グラフ対象に対応する確定済みデータが存在しない場合でも、自動で別対象へ切り替えない。

#### エラー表示
- 確定済み月が存在しない場合は、ダッシュボードの主要カードを説明のみの空状態表示に切り替える。
- 投資評価額が未入力の場合は、投資構成カードを静かな説明文による空状態表示に切り替える。
- 将来収支予測や投資判断が算出不可の場合は、理由のみを表示する。
- 通常資産全件モーダルは項目数が多い場合でもモーダル内スクロールで閲覧させる。
...
- モバイルでは期間切替ボタン群を横スクロールで扱う。
- 投資先一覧が長くなってもカード内スクロールにはせず、ページ全体で自然に縦方向へ伸ばす。
...
- 管理中の最後の月を確定した場合は、その次の1か月を新たに管理対象へ追加する。

#### ワイヤーフレーム
- タイトルカードの下に、対象月切替カードを配置する。
- 対象月切替カード内に `前月 / 年月プルダウン / 翌月` と、対象月の `確定済み / 未確定` ステータスを表示する。
...
| # | 項目名 | 型 | 必須 | 説明 |
|---|---|---|---|---|
| 1 | 表示対象月 | セレクト | - | 管理中の月から現在表示する年月を直接選択する。確定済み月では支出確定値もあわせて表示する。 |
| 2 | 月移動ボタン | ボタン | - | 前月・翌月へ移動する。年月プルダウン選択と同期する。 |
...
| # | アクション | トリガー | 処理内容 | API |
|---|---|---|---|---|
| 1 | 月切替 | 前月/翌月ボタン押下または年月プルダウン変更 | 表示対象月を切り替える。 | - |
