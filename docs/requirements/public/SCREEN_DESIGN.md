...
| 2 | 総資産 | KPIカード | - | 最新確定月の総資産を表示する。クリック操作は持たない。 |
...
| 6 | 資産サマリーカード | 情報カード | - | 総資産、収入、支出、投資評価額合計、生活防衛資金、通常資産上位3件を表示する。上位3件は総資産に対する比率付きで表示し、負債も絶対値で含め、`負債` ラベルと色分けで区別する。 |
| 7 | 通常資産全件モーダル導線 | ボタン | - | `残りn件` とあわせて、最新確定月で有効な通常資産全件を一覧だけのモーダルで開く。 |
