...
| 6 | 資産サマリーカード | 情報カード | - | 総資産、通常資産合計、投資評価額合計、収入、支出、生活防衛資金、通常資産上位3件を表示する。総資産は通常資産合計と投資評価額合計の合計として示し、通常資産上位3件は総資産内比率付きで表示する。負債も絶対値で含め、`負債` ラベルと色分けで区別する。 |
...
| 9 | 将来収支と投資判断カード | 情報カード | - | 予測月次収入、予測月次支出、平均対象月数、投資可能額、累積収入、累積支出、余力資産、採用イベントを表示する。採用イベントは `年月 + 内容` のみとし、複数イベント比較一覧は表示しない。平均対象月数は補足情報として扱う。 |
