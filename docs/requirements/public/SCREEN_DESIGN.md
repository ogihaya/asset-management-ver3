...
なお、`新規登録` と `パスワード再設定` のURLは、画面設計書上の仮固定値として `/signup`、`/password-reset` を採用している。
...
| 3 | SCR-AUTH-03 | パスワード再設定 | `/password-reset` | 不要 | ユーザー | メール経由でパスワード再設定を行う。 |
| 4 | SCR-APP-01 | ダッシュボード | `/dashboard` | 必要 | ユーザー | 最新確定月を基準に、資産状況、投資判断、投資構成、推移を確認する。 |
| 5 | SCR-APP-02 | 月次記録 | `/monthly-record` | 必要 | ユーザー | 月ごとの資産、収入、投資評価額を入力し、支出確認後に確定する。 |
| 6 | SCR-APP-03 | ライフプラン | `/life-plan` | 必要 | ユーザー | 将来イベントを年月、内容、金額、メモ単位で管理する。 |
| 7 | SCR-APP-04 | 設定 | `/settings` | 必要 | ユーザー | 生活防衛資金、投資先配分、将来収支予測前提を確認・管理する。 |
...
- 認証失敗またはセッション失効時は、入力欄付近に理由表示領域を出す。
...
| 6 | エラーメッセージ領域 | テキスト | - | ログイン失敗理由やセッション失効理由を表示する。 |
...
#### エラー表示
- 認証失敗時は、ログイン失敗の理由を画面内に表示する。
- セッション失効時は、再ログインが必要な理由を画面内に表示する。
...
- カード内にメールアドレス、パスワード、確認用パスワード、登録ボタンを縦積みで配置する。
...
| 3 | 確認用パスワード | パスワード入力 | ○ | パスワード再入力で一致確認に使う。 |
| 4 | 登録ボタン | ボタン | - | 登録処理を実行する。 |
| 5 | ログイン導線 | テキストリンク | - | ログイン画面へ戻る。 |
| 6 | エラーメッセージ領域 | テキスト | - | 入力不備や登録失敗理由を表示する。 |

#### アクション
| # | アクション | トリガー | 処理内容 | API |
|---|---|---|---|---|
| 1 | 新規登録 | 登録ボタン押下 | 入力値でユーザー登録を行う。 | API仕様書で定義 |
| 2 | ログインへ戻る | ログイン導線押下 | ログイン画面へ遷移する。 | - |

#### バリデーション
- メールアドレスは空欄不可、メール形式であること。
- パスワードは8文字以上かつ英字と数字を含むこと。
- 確認用パスワードはパスワードと一致すること。

#### エラー表示
- 既存メールアドレスで登録しようとした場合は理由を表示する。
- パスワード強度不足、形式不備、確認用パスワード不一致は入力欄近傍またはエラー領域で表示する。
...
### 3.3 パスワード再設定
#### 概要
- メール経由でパスワード再設定を開始し、完了させるための画面。
- 再設定開始と新パスワード入力の2状態を1画面内フローで扱う。

#### ワイヤーフレーム
- 中央カードレイアウトとする。
- 初期状態ではメールアドレス入力と送信ボタンを表示する。
- 再設定用入力状態では新パスワード、確認用パスワード、再設定ボタンを表示する。
...
| 3 | 新しいパスワード | パスワード入力 | 状態依存 | 新しいパスワードを入力する。 |
| 4 | 確認用パスワード | パスワード入力 | 状態依存 | 新しいパスワードの一致確認に使う。 |
| 5 | 再設定ボタン | ボタン | 状態依存 | パスワード再設定を完了する。 |
| 6 | ログイン導線 | テキストリンク | - | ログイン画面へ戻る。 |
| 7 | 結果メッセージ領域 | テキスト | - | 送信結果、再設定完了、失敗理由を表示する。 |

#### アクション
| # | アクション | トリガー | 処理内容 | API |
|---|---|---|---|---|
| 1 | 再設定開始 | 送信ボタン押下 | メール経由の再設定フローを開始する。 | API仕様書で定義 |
| 2 | パスワード再設定 | 再設定ボタン押下 | 新しいパスワードを保存する。 | API仕様書で定義 |
| 3 | ログインへ戻る | ログイン導線押下 | ログイン画面へ遷移する。 | - |
...
#### エラー表示
- 未登録メールアドレス、再設定失敗、リンク不正や期限切れなどの状態を画面内で表示する。
- 再設定完了時は完了メッセージを表示し、ログインへ戻れるようにする。

#### レスポンシブ対応
- ログイン画面と同様に、中央カード型の単一画面とする。
- スマホでは状態切替後も1画面内で操作できる構成とする。
...
| 6 | 資産サマリーカード | 情報カード | - | 総資産、収入、支出、投資評価額合計、生活防衛資金、通常資産上位3件を表示する。 |
| 7 | 通常資産全件モーダル導線 | ボタン | - | 通常資産全件をモーダルで開く。 |
| 8 | 投資構成カード | 情報カード | - | 最新確定月の投資評価額構成比円グラフ、実績比率、理想比率、差分、評価額を表示する。 |
| 9 | 将来収支と投資判断カード | 情報カード | - | 予測月次収入、予測月次支出、平均対象月数、投資可能額、累積収入、累積支出、余力資産、採用イベントを表示する。 |
| 10 | 資産グラフカード | グラフカード | - | 単一対象の資産推移グラフを表示する。 |
| 11 | 表示対象選択 | セレクト | - | 総資産または個別資産・投資先を選択する。 |
...
#### エラー表示
- 確定済み月が存在しない場合は、ダッシュボードの主要カードを空状態表示に切り替える。
...
- モバイルでは3カードを縦積みとする。
- 画面最下部に sticky の月次確定バーを配置し、未入力時は警告文を表示する。

#### 表示項目
| # | 項目名 | 型 | 必須 | 説明 |
|---|---|---|---|---|
| 1 | 表示対象月 | テキスト | - | 現在表示中の年月を示す。 |
| 2 | 月移動ボタン | ボタン | - | 前月・翌月へ移動する。 |
| 3 | 月次ステータス | ステータス表示 | - | 対象月が `確定済み / 未確定` のどちらかを示す。 |
| 4 | 資産入力カード | 一覧入力 | - | 通常資産の名称、金額、編集導線を表示する。 |
| 5 | 削除済み資産復活導線 | ボタン群 | - | 削除済み資産を復活する。 |
| 6 | 収入入力カード | 一覧入力 | - | 収入明細名、金額、編集導線を表示する。 |
| 7 | 投資入力カード | 一覧入力 | - | 設定済み投資先ごとの評価額入力欄を表示する。 |
...
| 13 | 月次確定バー | 固定アクションバー | - | 確定実行導線と注意文を表示する。 |
...
| 3 | 資産編集 | 編集アイコン押下 | 未確定月に限り、資産編集モーダルを開く。 | API仕様書で定義 |
| 4 | 資産削除 | 編集モーダル内削除押下 | 確認ダイアログを経て削除し、次の未確定月から非表示にする。 | API仕様書で定義 |
| 5 | 資産復活 | 復活ボタン押下 | 削除済み資産を最新確定月の次の未確定月から再表示する。 | API仕様書で定義 |
| 6 | 収入追加 | `追加` 押下 | 収入追加モーダルを開き、収入名を登録する。 | API仕様書で定義 |
| 7 | 収入編集 | 編集アイコン押下 | 収入編集モーダルを開く。 | API仕様書で定義 |
| 8 | 収入削除 | 編集モーダル内削除押下 | 確認ダイアログを経て削除する。 | API仕様書で定義 |
| 9 | 投資評価額入力 | 入力欄変更 | 設定済み投資先の当月評価額を更新する。 | API仕様書で定義 |
| 10 | 月次確定開始 | 月次確定ボタン押下 | 支出確認モーダルを開く。 | - |
| 11 | 月次確定 | 確定ボタン押下 | 支出補正後の値で月次を確定し、以後編集不可にする。 | API仕様書で定義 |

#### バリデーション
- 月次確定は古い未確定月から順番にのみ可能とする。
- 表示中の通常資産、収入、投資評価額のいずれかに空欄がある場合は確定不可とする。
- `0` は有効入力とし、空欄のみ未入力扱いとする。
- 投資入力カードは評価額のみ入力対象とし、投資先の追加・編集・削除導線は表示しない。
- 確定済み月では入力欄と編集導線を無効化する。

#### エラー表示
- 空欄がある場合は、月次確定バーおよび確認モーダルに警告文を表示する。
- 初月などで支出自動推定が計算不可の月は、確認モーダル内に理由を表示する。
- 確定期限超過時は対象月一覧とあわせて警告バナーを表示する。
...
- 将来収支予測は確認用表示であり、算出不可時は `算出不可` を表示する。
...
| ユーザーメニュー | 認証後全画面 | ユーザー情報確認とログアウト導線を表示する。 |
...
| トースト | 認証後全画面 | 保存成功や警告などの通知を右上に一定時間表示し、自動で閉じる。 |
